...
export type System = {
  ...
  planets: Planet[];
  fleets: Fleet[];
  freighters: Freighter[]; // Own freighters only.
  allFreighters: Freighter[]; // All freighters present this turn.
  unusedFreighters: Freighter[]; //These are put there per turn, and popped off when used..
  gates: Gate[];
  defence: number;
  owner: Civilization;
}
...
export namespace System {
  ...
  export function releaseFreightersForFleet(system: System, fleet: Fleet) {
    system.allFreighters = system.allFreighters.filter ((freighter) => freighter.owner != fleet.owner);
  }
  ...
  export function addToAllFreighters(system: System, freighter: Freighter) {
    if (!system.allFreighters.includes(freighter)) system.allFreighters.push(freighter);
  }

  export function berthOwnFreighter(system: System, freighter: Freighter) {
